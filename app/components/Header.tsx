    ...
    <header className="sticky top-0 z-50 border-b border-border-soft bg-background/80 backdrop-blur">
      <Container>
        {/* Mobile: wordmark and toggle on one row, nav wrapping under it.
            From sm: one row, wordmark then nav on the left, toggle right. */}
        <div className="flex flex-col gap-1 py-3 sm:min-h-16 sm:flex-row sm:items-center sm:gap-6 sm:py-2">
          <div className="flex items-center justify-between sm:contents">
            <Link
              href="/"
              aria-label="Stable Future — home"
              className="transition-transform duration-200 hover:scale-105 sm:order-1"
            >
          ...
          <nav aria-label="Primary" className="-mx-3 flex flex-wrap items-center gap-1 sm:order-2 sm:mx-0 sm:mr-auto">
            {NAV.map(([label, href]) => (
              <a
                key={href}
                href={href}
